...
*
* @property boolean $is_operator_off(string %pseudo)
*   - default: Retourne un boolean si le pseudo est un Opérateur Chat Skyrock.com officiel.
*
* @property string $nick.operator.sign(string %pseudo)
...
*
* @property string $nick.operator.sponsor(string %pseudo)
*   - default: Retourne le parrain d'un pseudo. (Opérateur)
*
* @property string $nick.color(string %pseudo, string %salon)
...
*/
alias -l nick.color._configDefault {
...
alias nick.color {
  ...
  if ($prop === custom) {
    if (!$is_skynaute(%nick)) {
      ...
      ; COULEURS:
      ;   First
      var %nfc = $iif($Configure::read(%config, first), $v1, %nick_color)
      ;   Between
      var %nbc = $iif($Configure::read(%config, between), $v1, %nick_color)
      ;   Last
      var %nlc = $iif($Configure::read(%config, last), $v1, %nick_color)
      ;   Status
      var %nsc = $iif($Configure::read(%config, status), $v1, %nick_color)

      if (%nick_status_only) {
        %nbc = $iif($Configure::read(%config, if_status__between), $v1, %nbc)
        %nfc = $iif($Configure::read(%config, if_status__first), $v1, %nfc)
        %nlc = $iif($Configure::read(%config, if_status__last), $v1, %nlc)
        %nsc = $iif($Configure::read(%config, if_status__status), $v1, %nsc)
      }

      if ($is_botoff(%nick)) {
        %nbc = $iif($Configure::read(%config, botoff__between), $v1, %nbc)
        %nfc = $iif($Configure::read(%config, botoff__first), $v1, %nfc)
        %nlc = $iif($Configure::read(%config, botoff__last), $v1, %nlc)
      }
      elseif ($nick.is_skyrock_vhost(%nick)) {
        %nbc = $iif($Configure::read(%config, skyrock_vhost__between), $v1, %nbc)
        %nfc = $iif($Configure::read(%config, skyrock_vhost__first), $v1, %nfc)
        %nlc = $iif($Configure::read(%config, skyrock_vhost__last), $v1, %nlc)
        %nsc = $iif($Configure::read(%config, skyrock_vhost__status), $v1, %nsc)
      }
...
*
* @example code:
*   echo $color(notice) -ae -> %nick $iif($is_operator_off(%nick), est un opérateur officiel SKYROCK.COM :-], n'est pas opérateur... :-[)
*
...
alias is_operator_off {
  ...
  return $iif(%group isnum && %group !== 1, $Configure::checkByIndex($nick.groups, %group), $false)
}
...
alias nick.operator.sign {
  ...
  var %group = $Configure::readByIndex(%groups, %group_num).key
  var %sign = $Configure::readByIndex(%groups, %group_num).value
